...
class MathGameM extends StatefulWidget {
  const MathGameM({super.key});

  @override
  State<MathGameM> createState() => _MathGameMState();
}

class _MathGameMState extends State<MathGameM> {
  int num1 = 0;
  int num2 = 0;
  List<int> options = [];
  int score = 0;
  ...
  void generateQuestion() {
    ...
    // Medium difficulty: numbers between 10 and 40
    num1 = rng.nextInt(30) + 10; 
    num2 = rng.nextInt(30) + 10; 
    int correctAns = num1 + num2;

    Set<int> optionSet = {correctAns};
    while (optionSet.length < 4) {
      int variance = rng.nextInt(16) - 8; // variance of -8 to +8
      int wrongAns = correctAns + (variance == 0 ? 3 : variance);
      if (wrongAns > 0) optionSet.add(wrongAns);
    }
  ...
  void _showGameOver() {
    showDialog(
      ...
      builder: (context) => AlertDialog(
        ...
        content: Text(
          "Final Score: $score\n" + 
          (_timeLeft <= 0 ? "Time's up!" : wrongAnswers >= maxWrong ? "Out of lives!" : "Medium Mode finished!"),
              ...
              child: Column(
                ...
                children: [
                  Text(
                    ...
                    style: const TextStyle(fontFamily: 'Pixellari', color: Colors.black, fontSize: 14, fontWeight: FontWeight.bold),
                  ),
                  const SizedBox(height: 5),
                  Text(
                    "SCORE: $score",
                    style: const TextStyle(fontFamily: 'Pixellari', color: Colors.black, fontSize: 22, fontWeight: FontWeight.bold),
                  ),
                ],
              ),
            ),
            ...
            Positioned(
              ...
              child: Column(
                children: [
                  Text(
                    "TIME: $_timeLeft",
                    style: const TextStyle(fontFamily: 'Pixellari', fontSize: 18, color: Colors.redAccent, fontWeight: FontWeight.bold),
                  ),
                  const SizedBox(height: 5),
                  Text(
                    "WHAT IS $num1 + $num2?",
                    textAlign: TextAlign.center,
                    style: const TextStyle(fontFamily: 'Pixellari', fontSize: 30, color: Colors.black, fontWeight: FontWeight.bold),
                  ),
                ],
              ),
            ),
